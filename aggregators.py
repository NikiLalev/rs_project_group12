...
class BaselinesAggregator(AggregationStrategy):
    def generate_group_recommendations_for_group(self, group_ratings, recommendations_number):
        ...
        # approved_items = group_ratings.loc[group_ratings['rating'] >= approval_threshold, 'item']
        # approval_counts = approved_items.value_counts().reset_index()
        # approval_counts.columns = ['item', 'vote_count']
        # approval_recommendation_list = list(approval_counts.sort_values(by='vote_count', ascending=False).head(recommendations_number)['item'])

        # Plurality
        group_ratings_clean = group_ratings.dropna(subset=['rating'])
        top_ratings = group_ratings_clean.loc[group_ratings_clean.groupby('user')['rating'].idxmax()]

        plurality_counts = top_ratings['item'].value_counts().reset_index()
        plurality_counts.columns = ['item', 'vote_count']
        plurality_recommendation_list = list(plurality_counts.sort_values(by='vote_count', ascending=False).head(recommendations_number)['item'])
        ...
        return {
            ...
            # "APP":approval_recommendation_list,
            "PLU":plurality_recommendation_list
        }
